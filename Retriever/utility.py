...
from retriever.config_loader import *
import ast
import requests
import re
import numpy as np
    ...
    # try:
        ...
        for item in node_context:
            try:
                ...
                print(f"TypeError encountered: {e}. Skipping item: {item}")
                continue  # 如果遇到TypeError，跳过当前项并继续处理其他项
...
def retrieve_context(question, vectorstore, embedding_function, node_context_df, context_volume, context_sim_threshold, context_sim_min_threshold, edge_evidence, api=True):
    ...
    if entities is None or len(entities) == 0:
        return "No disease entities found in the question."
    node_hits = []
    if entities:
        ...
        for node_name in node_hits:
            try:
                ...
                if not node_context:  # 如果 get_context_using_spoke_api 返回空字符串，则跳过后续处理
                    continue
                ...
                if edge_evidence:
                    ...
                    merged_contexts = process_context(high_similarity_context)
                    node_context_extracted += ". ".join(merged_contexts)
                    # node_context_extracted += ". ".join(high_similarity_context)
                    node_context_extracted += ". "
            except Exception as e:
                print(f"Error occurred while processing node '{node_name}': {e}")
                continue
...
def process_context(context_list):
    ...
    pattern = r"(Disease [\w\s]+)\s(presents|localizes|resembles)\s([\w\s]+)\s(.*)"
    
    for context in context_list:
        match = re.match(pattern, context)
        if match:
            subject, predicate, object_type, object_value = match.groups()
            subject = subject.strip()
            
            if predicate == "presents":
                relations[subject]["symptoms"].add(object_value.strip())
            elif predicate == "localizes":
                relations[subject]["anatomy"].add(object_value.strip())
            elif predicate == "resembles":
                relations[subject]["similar_diseases"].add(object_value.strip())
    
    processed_context = []
    for subject, predicates in relations.items():
        subject_parts = [subject]
        
        if "symptoms" in predicates:
            symptoms = ", ".join(sorted(predicates["symptoms"]))
            subject_parts.append(f"Presents symptoms: {symptoms}")
        
        if "anatomy" in predicates:
            anatomy = ", ".join(sorted(predicates["anatomy"]))
            subject_parts.append(f"Localizes in: {anatomy}")
        
        if "similar_diseases" in predicates:
            similar = ", ".join(f"Disease {disease}" for disease in sorted(predicates["similar_diseases"]))
            subject_parts.append(f"Resembles: {similar}")
        
        processed_context.append(" ".join(subject_parts))
