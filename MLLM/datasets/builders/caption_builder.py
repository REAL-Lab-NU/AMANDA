...
from MLLM.datasets.datasets.coco_caption_datasets import (
    COCOCapDataset,
    COCOCapEvalDataset,
    NoCapsEvalDataset,
)

from MLLM.datasets.datasets.caption_datasets import TextCapsDataset, CaptionDataset, LLaVAPretrainDataset
from MLLM.datasets.datasets.llava_dataset import MEDLLAVADataset, MEDPretrainDataset
...
@registry.register_builder("medllava_pretrain")
class MEDLLaVAPretrainBuilder(BaseDatasetBuilder):
    train_dataset_cls = MEDPretrainDataset

    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/medllava/pretrain_558.yaml",
    }
    ...
    def build_datasets(self):
        ...
        datasets['train'] = dataset_cls(
            vis_processor=self.vis_processors["train"],
            text_processor=self.text_processors["train"],
            ann_paths=[os.path.join(storage_path, 'llava_med_alignment_500k_valid.json')], 
            vis_root=vis_root,
        )
