...
from MLLM.datasets.builders.base_dataset_builder import BaseDatasetBuilder
from MLLM.common.registry import registry
from MLLM.datasets.datasets.aok_vqa_datasets import AOKVQADataset, AOKVQAEvalDataset, VQGAOKVQADataset
from MLLM.datasets.datasets.coco_vqa_datasets import COCOVQADataset, COCOVQAEvalDataset, VQGCOCOVQADataset
from MLLM.datasets.datasets.ocr_vqa_dataset import OCRVQADataset, STVQADataset, DocVQADataset
from MLLM.datasets.datasets.llava_dataset import LLAVADataset
from MLLM.datasets.datasets.llava_dataset import MEDLLAVAConversationDataset, MEDLLAVADataset, MEDPretrainDataset
...
@registry.register_builder("medllavavqa") 
class MEDLLAVABuilder(BaseDatasetBuilder):
    train_dataset_cls = MEDLLAVAConversationDataset

    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/medllava/defaults.yaml",
    }
    ...
    def build_datasets(self):
        ...
        datasets['train'] = dataset_cls(
            vis_processor=self.vis_processors["train"],
            text_processor=self.text_processors["train"],
            ann_paths=[os.path.join(storage_path, 'BioMed-VITAL-instructions-150K-valid.json')], 
            vis_root=vis_root,
        )
...
@registry.register_builder("medvqa") 
class MEDVQABuilder(BaseDatasetBuilder):
    train_dataset_cls = MEDLLAVADataset
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/medllava/vqa.yaml",
    }
    ...
    def build_datasets(self):
        ...
        datasets['train'] = dataset_cls(
            vis_processor=self.vis_processors["train"],
            text_processor=self.text_processors["train"],
            ann_paths=[os.path.join(storage_path, 'train_rad.json')], 
            vis_root=vis_root,
        )
...
class RADVQABuilder(BaseDatasetBuilder):
    train_dataset_cls = MEDLLAVADataset
    eval_dataset_cls = MEDLLAVADataset
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/medllava/rad.yaml",
    }
    ...
    def build_datasets(self):
        ...
        dataset_cls_eval = self.eval_dataset_cls
        datasets['val'] = dataset_cls_eval(
            vis_processor=self.vis_processors["eval"],
            text_processor=self.text_processors["eval"],
            ann_paths=[os.path.join(storage_path, 'test_rad.json')], 
            vis_root=vis_root,
        )

        return datasets
    
@registry.register_builder("slakevqa") 
class SLAKEVQABuilder(BaseDatasetBuilder):
    train_dataset_cls = MEDLLAVADataset
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/medllava/slake.yaml",
    }
    ...
    def build_datasets(self):
        ...
        datasets['train'] = dataset_cls(
            vis_processor=self.vis_processors["train"],
            text_processor=self.text_processors["train"],
            ann_paths=[os.path.join(storage_path, 'train_slake.json')], 
            vis_root=vis_root,
        )
...
@registry.register_builder("pvqavqa") 
class PVQAVQABuilder(BaseDatasetBuilder):
    train_dataset_cls = MEDLLAVADataset
    DATASET_CONFIG_DICT = {
        "default": "configs/datasets/medllava/pvqa.yaml",
    }
